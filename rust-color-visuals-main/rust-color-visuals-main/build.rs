...
fn main() {
    ...
    let title = user_cfg
        .title
        .unwrap_or_else(|| "Perlin Flow Particles".to_string());
    ...
    // Generate Rust module directly into src so editor/linter can see it
    let dest_path = manifest_dir.join("src").join("build_config.rs");
    let contents = format!(
    ...
    fs::write(dest_path, contents).expect("failed to write src/build_config.rs");
}
